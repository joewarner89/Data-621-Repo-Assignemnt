...
transformData <- function(data ,fields,methods){
  ...
  rm_lambda <- data |> select(all_of(fields)) |> preProcess(method = methods)
  trasnformed <- predict(rm_lambda,data)
  
  return(list(trasnformed,rm_lambda))
}
...
#### Probability Distributions of training and test data (TODO)

To evaluate model performance, we compared the predicted probabilities of each model against the distribution of prediction probabilities in the test data, allowing us to infer the generalization capability and robustness of each model.
...
```{r}
# ggplot(comparison_df, aes(x = predicted_prob, fill = modeltype)) +
#   geom_density(alpha = 0.5) + facet_wrap(~modelNumber, scales = 'free', ncol = 2) +
...
```{r}
set.seed(123456)
crime_training_cross1 <- crime_training_m1
crime_training_cross1$expected <- factor(crime_training_cross1$target,levels = c(0,1),labels = c("no","yes") )

crime_training_cross1 <- crime_training_cross1 |> select(-target)

control <- trainControl(method = "repeatedcv",  repeats = 5, number = 5, classProbs = TRUE, summaryFunction = twoClassSummary)
cv_model <- train(expected~nox+age+ptratio+dis+rad, data = crime_training_cross1, method = "glm", family = "binomial", trControl = control, metric = "ROC")

cv_model_result_1 <- cv_model$resample |> separate(Resample,into = c("Ford","Repeat"))
...
```{r}
ggplot(cv_model_result_1, aes(x = factor(Ford), y = ROC, color = factor(Repeat))) +
  geom_point(size = 3) +
  geom_line(aes(group = Repeat), linetype = "dashed") +
  labs(title = "Cross-Validation ROC Across Folds and Repeats",
       x = "Fold",
       y = "ROC AUC") +
  theme_minimal()
```
...
```{r}
ggplot(cv_model_result_1, aes(x = factor(Ford), y = Sens, color = factor(Repeat))) +
  geom_point(size = 3) +
  geom_line(aes(group = Repeat), linetype = "dashed") +
  labs(title = "Cross-Validation Sensitivity Across Folds and Repeats",
       x = "Fold",
       y = "ROC AUC") +
  theme_minimal()
```
...
```{r}
ggplot(cv_model_result_1, aes(x = factor(Ford), y = Spec, color = factor(Repeat))) +
  geom_point(size = 3) +
  geom_line(aes(group = Repeat), linetype = "dashed") +
  labs(title = "Cross-Validation Specificity Across Folds and Repeats",
       x = "Fold",
       y = "ROC AUC") +
  theme_minimal()
```
