...
### 4.2 4.2 Binary Logistic Regression Model
...
```{r}
# Convert TARGET_FLAG to a factor in both datasets
insurance_training$TARGET_FLAG <- factor(insurance_training$TARGET_FLAG)
insurance_evaluation$TARGET_FLAG <- factor(insurance_evaluation$TARGET_FLAG)
```


```{r}
# Check levels after conversion
levels(insurance_training$TARGET_FLAG)
levels(insurance_evaluation$TARGET_FLAG)
```


```{r}
# Relevel the TARGET_FLAG in the evaluation dataset to match the training dataset
insurance_evaluation$TARGET_FLAG <- factor(insurance_evaluation$TARGET_FLAG, levels = levels(insurance_training$TARGET_FLAG))
```


```{r}
# Logistic Regression Model (for TARGET_FLAG)
log_model1 <- glm(TARGET_FLAG ~ AGE + CAR_AGE + KIDSDRIV_LOG + HOMEKIDS_LOG, 
                  family = binomial, data = insurance_training)

# AIC and Log-Likelihood
log_model1_aic <- AIC(log_model1)
log_model1_ll <- logLik(log_model1)

# Make predictions
log_model1_preds <- predict(log_model1, newdata = insurance_evaluation, type = "response")
log_model1_class <- ifelse(log_model1_preds > 0.5, 1, 0)  # Convert probabilities to binary predictions

# Confusion Matrix
library(caret)
conf_matrix <- confusionMatrix(as.factor(log_model1_class), as.factor(insurance_evaluation$TARGET_FLAG))

# Display Metrics
cat("AIC: ", log_model1_aic, "\n")
cat("Log-Likelihood: ", log_model1_ll, "\n")
...
# ROC Curve and AUC
roc_curve <- roc(insurance_evaluation$TARGET_FLAG, log_model1_preds)
