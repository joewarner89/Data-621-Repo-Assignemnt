...
training_wine_data <- training_wine_data %>%
  mutate(across(where(is.numeric), ~ ifelse(is.na(.), median(., na.rm = TRUE), .)))

# Create missing flags
missing_flags <- training_wine_data %>%
...
training_wine_data <- training_wine_data %>%
  mutate(acidity_ratio = FixedAcidity / VolatileAcidity)
...
poisson_model2 <- glm(TARGET ~ log_target + acidity_ratio, family = poisson(link = "log"), data = training_wine_data)

# Negative Binomial models
library(MASS)
nb_model1 <- glm.nb(TARGET ~ ., data = training_wine_data)
nb_model2 <- glm.nb(TARGET ~ log_target + acidity_ratio, data = training_wine_data)

# Linear regression models
lm_model1 <- lm(TARGET ~ ., data = training_wine_data)
lm_model2 <- lm(TARGET ~ log_target + acidity_ratio, data = training_wine_data)
